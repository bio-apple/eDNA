...
db=args.ref.split("/")[-1]
taxonomy=cmd+f"usearch -sintax /outdir/{args.prefix}.zotus.fasta -db /ref/{db} -strand both -tabbedout /outdir/{args.prefix}.zotus.sintax_results.txt -sintax_cutoff 0.8\'"
print(taxonomy)
...
#uses the SINTAX algorithm to predict taxonomy for query sequences
db=args.ref.split("/")[-1]
taxonomy=cmd+f"usearch -sintax /outdir/{args.prefix}.otus.fasta -db /ref/{db} -strand both -tabbedout /outdir/{args.prefix}.otus.sintax_results.txt -sintax_cutoff 0.8\'"
print(taxonomy)
subprocess.check_call(taxonomy,shell=True)
