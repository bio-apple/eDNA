...
def run(R1,R2,prefix,outdir,ref,type,primer,name):
    ...
    with open(f"{outdir}/{prefix}.Rscript","w") as script:
        ...
        script.write(
            ...
            #maxEE=c(2.75,5.5)
            f"out<-filterAndTrim(fnFs,filtFs, fnRs,filtRs,maxN=0,trimLeft=c({left}, {right}),minLen=100,maxEE=c(2,2),rm.phix=TRUE,compress=TRUE,multithread=TRUE)\n"
            #f"out<-filterAndTrim(fnFs,filtFs, fnRs,filtRs,maxN=0,maxEE=c(2.75,5.5),rm.phix=TRUE,compress=TRUE,multithread=TRUE)\n"
