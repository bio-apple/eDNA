...
parser.add_argument("-r","--ref",help="qiime reference",required=True)
...
derep=cmd+f"vsearch --derep_fulllength /outdir/{args.prefix}.filtered.fasta --output /outdir/{args.prefix}.uniques.fasta -relabel Uniq -sizeout --minuniquesize 10\'"
print(derep)
...
zotus=cmd+(f"usearch -unoise3 /outdir/{args.prefix}.uniques.fasta -zotus /outdir/{args.prefix}.zotus.fasta -tabbedout /outdir/{args.prefix}.unoise3.txt && "
           f"qiime tools import --type \'FeatureData[Sequence]\' --input-path /outdir/{args.prefix}.zotus.fasta --output-path /outdir/{args.prefix}.zotus.qza && "
          f"qiime feature-classifier classify-sklearn --i-classifier /ref/{ref} --i-reads /outdir/{args.prefix}.zotus.qza --o-classification /outdir/{args.prefix}.zotus.taxonomy.qza && "
          f"qiime tools export --input-path /outdir/{args.prefix}.zotus.taxonomy.qza --output-path /outdir/{args.prefix}_zotus_taxonomy\'")
print(zotus)
subprocess.check_call(zotus,shell=True)

#OTU
otu=cmd+(f"usearch -cluster_otus /outdir/{args.prefix}.uniques.fasta -otus /outdir/{args.prefix}.otus.fasta -relabel Otu && "
         f"qiime tools import --type \'FeatureData[Sequence]\' --input-path /outdir/{args.prefix}.otus.fasta --output-path /outdir/{args.prefix}.otus.qza && "
          f"qiime feature-classifier classify-sklearn --i-classifier /ref/{ref} --i-reads /outdir/{args.prefix}.otus.qza --o-classification /outdir/{args.prefix}.otus.taxonomy.qza && "
          f"qiime tools export --input-path /outdir/{args.prefix}.otus.taxonomy.qza --output-path /outdir/{args.prefix}_otus_taxonomy\'")
print(otu)
