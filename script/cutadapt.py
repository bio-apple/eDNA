...
def run(R1, R2,primer,name,prefix,outdir,read):
    ...
    if R2!="None":
        ...
        if os.path.dirname(R1) != os.path.dirname(R2):
            ...
            if read < amplicon:
                ...
                cmd+=f"cutadapt -j 16 --match-read-wildcards --max-n 2 -g {forward} -G {reverse} --report=full --discard-untrimmed -o /outdir/{prefix}_no_primer_R1.fastq.gz -p /outdir/{prefix}_no_primer_R2.fastq.gz /raw_data/{R1.split('/')[-1]} /raw_data/{R2.split('/')[-1]}\'"
            else:
                ...
                cmd+=f"cutadapt -j 16 --match-read-wildcards --max-n 2 -g {forward} -G {reverse} -a {reverse_forward_c} -A {forward_rev_c} --report=full --discard-untrimmed -o /outdir/{prefix}_no_primer_R1.fastq.gz -p /outdir/{prefix}_no_primer_R2.fastq.gz /raw_data/{R1.split('/')[-1]} /raw_data/{R2.split('/')[-1]}\'"
    else:
        # single read
        if read > amplicon:
            cmd+=f"cutadapt -j 16 --match-read-wildcards --max-n 2 -g {forward} -a {reverse_forward_c} --report=full --discard-untrimmed -o /outdir/{prefix}_no_primer_R1.fastq.gz /raw_data/{R1.split('/')[-1]}\'"
    print(cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--primer', required=True, type=str, help='primer file')
    ...
    run(args.pe1, args.pe2,args.primer,args.name,args.prefix,args.outdir,args.len)
